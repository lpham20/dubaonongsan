...
const topicToCrop: Array<{ match: string[]; crop: CropType; label: string }> = [
  { match: ["cà phê", "ca phe", "robusta", "arabica"], crop: "ca_phe", label: toNFC("cà phê") },
  { match: ["sầu riêng", "sau rieng", "durian"], crop: "sau_rieng", label: toNFC("sầu riêng") },
  { match: ["hồ tiêu", "ho tieu", "tiêu", "pepper"], crop: "ho_tieu", label: toNFC("hồ tiêu") },
  { match: ["lúa", "lua", "gạo", "gao", "rice"], crop: "lua", label: toNFC("lúa") }
];
  ...
  return (
    <aside className="related-forecast">
      ...
      <Link to={withLanguagePrefix(forecastPath(matched.crop), language)}>
        <BarChart3 size={16} />
        {toNFC(language === "en" ? `${label} price today and 30-day forecast` : `Giá ${matched.label} hôm nay và dự báo 30 ngày`)}
        <ArrowRight size={15} />
      </Link>
      <Link to={withLanguagePrefix("/huong-dan", language)}>
        <BookOpenCheck size={16} />
        {toNFC(language === "en" ? `${label} farming guides` : `Hướng dẫn kỹ thuật trồng ${matched.label}`)}
        <ArrowRight size={15} />
      </Link>
      <Link to={withLanguagePrefix("/tin-tuc", language)}>
        <Newspaper size={16} />
        {toNFC(language === "en" ? `Latest ${label} news` : `Tin tức ${matched.label} mới nhất`)}
        <ArrowRight size={15} />
      </Link>
    </aside>
  );
